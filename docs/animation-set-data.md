...
- **the moving-attack flag** (§4.6) is derived from nothing; `setgen --flags-from` copies it.
  ...
  **6 use the flag** — the player's two, the werewolf, the netch, the witchlight and the
  ...
  | a blend, bone switch or `BSCyclicBlendTransitionGenerator` between the named clip and the root — the attack mixed with what was already playing | looked like the best of them until every parent was followed: **a node in a behaviour has more than one parent** (11.1% of nodes, 6.8% of clip generators), and `ProjectWalk` keeps the first the walk saw. Down one arbitrary path it fires on 17 flagged and 42 clear; over every path, 29 flagged and 76 clear; over all paths at once, 11 and 33. The version that finds most of the flagged finds a quarter of the clear ones with them |
